...
class nusoap_base_class {
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
    	...
    	if (is_object($val) && get_class($val) == 'soapval_class' && (! $soapval)) {
		...
		switch(true) {
			...
			case is_object($val):
		    	...
		    	if (get_class($val) == 'soapval_class') {
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){
					...
					if(is_array($val) && count($val)> 0){
						foreach($val as $v){
	                    	if(is_object($v) && get_class($v) ==  'soapval_class'){
								$tt_ns = $v->type_ns;
								$tt = $v->type;
							} elseif (is_array($v)) {
...
* @access public
*/
class nusoap_fault_class extends nusoap_base_class {
	/**
 ...
 */
class soap_fault extends nusoap_fault_class {
}
...
* @access   public
*/
class nusoap_xmlschema_class extends nusoap_base_class  {
 ...
 * Backward compatibility
 */
class XMLSchema extends nusoap_xmlschema_class {
}
...
* @access   public
*/
class soapval_class extends nusoap_base_class {
	/**
...
* @access public
*/
class soap_transport_http_class extends nusoap_base_class {
...
/**
*
* nusoap_server_class allows the user to create a SOAP server
...
* @access   public
*/
class nusoap_server_class extends nusoap_base_class {
	/**
	...
	*/
	function serialize_return() {
		...
		// if fault
		if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'soap_fault') || (get_class($this->methodreturn) == 'nusoap_fault_class'))) {
	...
	*/
    function parseRequest($headers, $data) {
		...
		$parser = new nusoap_parser_class($data,$this->xml_encoding,'',$this->decode_utf8);
		// parser debug
    ...
    */
    function configureWSDL($serviceName,$namespace = false,$endpoint = false,$style='rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
        ...
        $this->wsdl->schemas[$schemaTargetNamespace][0] = new nusoap_xmlschema_class('', '', $this->wsdl->namespaces);
        if ($style == 'document') {
 ...
 * Backward compatibility
 */
class soap_server extends nusoap_server_class {
}
...
* @access public 
*/
class wsdl_class extends nusoap_base_class {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
        	...
        	// get wsdl
	        $tr = new soap_transport_http_class($wsdl, $this->curl_options, $this->use_curl);
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->status = 'schema';
            $this->currentSchema = new nusoap_xmlschema_class('', '', $this->namespaces);
            $this->currentSchema->schemaStartElement($parser, $name, $attrs);
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
        } else {
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
    	...
    	if (is_object($value) && get_class($value) == 'soapval_class') {
    		if ($value->type_ns) {
...
/**
*
* nusoap_parser_class class parses SOAP XML messages into native PHP values
*
* @author   Dietrich Ayala <dietrich@ganx4.com>
* @author   Scott Nichol <snichol@users.sourceforge.net>
* @version  $Id: nusoap.php,v 1.123 2010/04/26 20:15:08 snichol Exp $
* @access   public
*/
class nusoap_parser_class extends nusoap_base_class {
 ...
 * Backward compatibility
 */
class soap_parser extends nusoap_parser_class {
}
...
* $soapclient = new nusoap_client_class( string path [ ,mixed wsdl] );
*
...
* @access   public
*/
class nusoap_client_class extends nusoap_base_class  {
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($headers){$this->requestHeaders = $headers;}
	...
	*/
	...
	function loadWSDL() {
		...
		$this->wsdl = new wsdl_class('',$this->proxyhost,$this->proxyport,$this->proxyusername,$this->proxypassword,$this->timeout,$this->response_timeout,$this->curl_options,$this->use_curl);
		...
		switch(true){
			...
			case preg_match('/^http/',$this->endpoint):
				...
				if($this->persistentConnection == true && is_object($this->persistentConnection)){
					...
					$http = new soap_transport_http_class($this->endpoint, $this->curl_options, $this->use_curl);
					if ($this->persistentConnection) {
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
		$parser = new nusoap_parser_class($data,$this->xml_encoding,$this->operation,$this->decode_utf8);
...
if (!extension_loaded('soap')) {
	 ...
	 */
	class soapclient extends nusoap_client_class {
	}
}
?>